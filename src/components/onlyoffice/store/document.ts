...
export type SetOnlyOfficeDocumentStateInput = Omit<OnlyOfficeDocumentState, 'isNew' | 'fileName'> & {
  isNew?: boolean
  fileName: string
}
...
function normalizeDocumentState(state: SetOnlyOfficeDocumentStateInput): OnlyOfficeDocumentState {
  return {
    ...state,
...
export function setDocumentObj(state: SetOnlyOfficeDocumentStateInput, scopeId?: string) {
...
export function setDocumentFile(file: File, fileName = file.name, scopeId?: string) {
  setDocumentObj(
    {
      isNew: false,
      file,
