...
import { type CreateEditorViewOptions, editorManagerFactory } from '../core/editor-manager'
import type { EditorLogger } from '../internal/editor/logger'
import { ensureTitleWithExtension, getX2tExportFormats, normalizeX2tExportFileType } from '../internal/editor/utils'
...
export async function createEditorView(options: CreateEditorViewOptions) {
  const manager =
    ...
    (options.containerId ? editorManagerFactory.get(options.containerId) : editorManagerFactory.getDefault())
