...
function resetLoadedDocsApi(apiUrl: string) {
  ...
  document
    .querySelectorAll<HTMLScriptElement>('script[src*="/web-apps/apps/api/documents/api.js"]')
    .forEach((script) => script.remove())
  document.querySelectorAll<HTMLIFrameElement>('iframe[data-onlyoffice-preload]').forEach((iframe) => iframe.remove())
