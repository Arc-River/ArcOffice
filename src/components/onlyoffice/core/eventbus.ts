...
export type OfficeXmlSizeLimitExceededData = OfficeXmlSizeLimitExceededPayload & {
  instanceId: string
  containerId: string
}
...
class OnlyOfficeEventBus {
  ...
  on<Key extends keyof OnlyOfficeEventPayloads>(key: Key, handler: (data: OnlyOfficeEventPayloads[Key]) => void) {
    this.emitter.on(key, handler as EventListener)
  }

  off<Key extends keyof OnlyOfficeEventPayloads>(key: Key, handler: (data: OnlyOfficeEventPayloads[Key]) => void) {
    this.emitter.off(key, handler as EventListener)
  }

  emit<Key extends keyof OnlyOfficeEventPayloads>(key: Key, data: OnlyOfficeEventPayloads[Key]) {
    this.emitter.emit(key, data)
  }

  waitFor<Key extends keyof OnlyOfficeEventPayloads>(key: Key, timeout = 30000): Promise<OnlyOfficeEventPayloads[Key]> {
