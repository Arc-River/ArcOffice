...
import type { OfficeTheme, OnlyOfficeConnector, OnlyOfficeConnectorOptions, User } from '../internal/editor/types'
import { clearDocumentObj, getDocumentObj, setDocumentObj } from '../store/document'
import { getCurrentLang, getOnlyOfficeLang, type OnlyOfficeLang, setCurrentLang } from '../store/lang'
...
import { type EditorManager, editorManagerFactory } from './editor-manager'
import { type LoadingChangeData, type OfficeXmlSizeLimitExceededData, onlyofficeEventbus } from './eventbus'
...
export class OnlyOfficeManager {
  ...
  private constructor(editor: EditorManager, options: OnlyOfficeManagerOptions & { containerId: string }) {
  ...
  static async create(options: OnlyOfficeManagerOptions): Promise<OnlyOfficeManager> {
  ...
  static async createWithFile(options: OnlyOfficeManagerOptions, file: File): Promise<OnlyOfficeManager> {
    const containerId = options.containerId ?? ONLYOFFICE_ID
    await initializeOnlyOffice()
  ...
  async openDocument(input: OpenDocumentInput) {
    ...
    await this.editor.create({
      file,
      fileName,
      // Only pass preset fileType for new documents; existing files auto-detect from filename
      fileType: !file ? this.fileType : undefined,
      isNew: !file,
      readOnly,
  ...
  async toggleLanguage() {
    const nextLang = getCurrentLang() === ONLYOFFICE_LANG_KEY.ZH ? ONLYOFFICE_LANG_KEY.EN : ONLYOFFICE_LANG_KEY.ZH
  ...
  /** 在浅色 / 深色主题之间切换 */
  async toggleTheme() {
    const darkThemes: OfficeThemeId[] = [OFFICE_THEME.DARK, OFFICE_THEME.NIGHT, OFFICE_THEME.CONTRAST_DARK]
    const nextTheme = darkThemes.includes(this.getTheme() as OfficeThemeId) ? OFFICE_THEME.WHITE : OFFICE_THEME.DARK
  ...
  onOfficeXmlSizeLimitExceeded(handler: (data: OfficeXmlSizeLimitExceededData) => void) {
    onlyofficeEventbus.on(ONLYOFFICE_EVENT_KEYS.OFFICE_XML_SIZE_LIMIT_EXCEEDED, handler)
    return () => {
      onlyofficeEventbus.off(ONLYOFFICE_EVENT_KEYS.OFFICE_XML_SIZE_LIMIT_EXCEEDED, handler)
    }
  }
...
export class OnlyOfficeManagerFactory {
  ...
  async open(options: OnlyOfficeManagerOptions, document: OpenDocumentInput): Promise<OnlyOfficeManager> {
    const containerId = options.containerId ?? ONLYOFFICE_ID
