...
function getFileType(fileName: string, fileType?: string) {
  // Explicit fileType takes priority
  if (fileType) return fileType.toLowerCase()
  // Extract extension from filename; fall back to "docx" if no dot present
...
export class EditorManager {
  ...
  private crossOriginCommentRefreshPromise: Promise<CommentItem[]> | null = null
  private revisions: RevisionItem[] = []
  private refreshingRevisions = false
  private crossOriginRevisionRefreshPromise: Promise<RevisionItem[]> | null = null
  ...
  private officeXmlSizeLimitPayload: OfficeXmlSizeLimitExceededPayload | null = null
  ...
  private getOfficeOverlayHostElement() {
    ...
    return container?.closest<HTMLElement>(ONLYOFFICE_CONTAINER_CONFIG.PARENT_SELECTOR) ?? container
  }
  ...
  private renderOfficeXmlSizeLimitOverlay() {
    ...
    const container = this.getOfficeOverlayHostElement()
    if (container) {
      this.officeXmlSizeLimitOverlayTeardown = showOfficeXmlSizeLimitOverlay(container, this.officeXmlSizeLimitPayload)
    }
  }
  ...
  private handleServerLoadError(error: Error) {
    ...
    try {
      onlyofficeEventbus.emit(ONLYOFFICE_EVENT_KEYS.OFFICE_XML_SIZE_LIMIT_EXCEEDED, data)
    } catch (eventError) {
      console.error('[EditorManager] officeXmlSizeLimitExceeded handler failed', eventError)
    }
  }
  ...
  private getEditorFrameElement() {
    ...
    const frames = Array.from(document.querySelectorAll<HTMLIFrameElement>('iframe[name="frameEditor"]'))
  ...
  private installIframeProxies() {
    ...
    const win = iframe.contentWindow as (OnlyOfficeWindow & ReporterHookWindow) | undefined
  ...
  private applyDefaultReviewSettings() {
    ...
    if (isOnlyOfficeCdnMode()) {
      void this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_SET_TRACK, {
        enabled: false,
      }).catch(() => {})
      return
    }
  ...
  private normalizeCrossOriginComments(value: unknown): CommentItem[] {
    ...
    return value.map((item, index) => {
      const source = item && typeof item === 'object' ? (item as Record<string, unknown>) : {}
      const data = source.Data && typeof source.Data === 'object' ? (source.Data as CommentData) : {}
  ...
  private refreshCrossOriginComments() {
    ...
    this.crossOriginCommentRefreshPromise = this.callCrossOriginComment(CROSS_ORIGIN_EDITOR_COMMAND.COMMENT_LIST, {})
      .then((items) => {
  ...
  private refreshRevisionsFromSdk(options?: { forceRefreshStack?: boolean }) {
    ...
    try {
      this.revisions = collectRevisionItems(api as RevisionsEditorApi, frameWin, options)
    } finally {
  ...
  private applyRevisionsFromSdkStack(stack: unknown) {
    ...
    this.revisions = resolveRevisionShowChanges(stack, api as RevisionsEditorApi, frameWin)
  }
  ...
  private applyAllRevisionChanges(mode: 'accept' | 'reject') {
    ...
    const applyBulk = mode === 'accept' ? () => api.asc_AcceptChanges?.() : () => api.asc_RejectChanges?.()
  ...
  private normalizeCrossOriginRevisions(value: unknown): RevisionItem[] {
    ...
    return value.map((item, index) => {
      const source = item && typeof item === 'object' ? (item as Record<string, unknown>) : {}
      const data = source.Data && typeof source.Data === 'object' ? (source.Data as RevisionItem['Data']) : {}

      return {
        Id: String(source.Id ?? `rev-stack-${index}`),
        Index: typeof source.Index === 'number' && Number.isFinite(source.Index) ? source.Index : index,
        Data: data,
  ...
  private refreshCrossOriginRevisions(options?: { forceRefreshStack?: boolean }) {
    if (this.crossOriginRevisionRefreshPromise) {
      return this.crossOriginRevisionRefreshPromise
    }

    this.crossOriginRevisionRefreshPromise = this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_LIST, {
  ...
  private callCrossOriginRevision(command: string, payload: Record<string, unknown> = {}) {
  ...
  addDemoRevision(text = `审批修订 ${new Date().toLocaleTimeString()}`) {
    ...
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_ADD_DEMO, { text }).then((items) => {
        this.revisions = this.normalizeCrossOriginRevisions(items)
        return this.revisions
      })
    }
    ...
    this.wordContentSyncPromise = (async () => {
      if (isOnlyOfficeCdnMode()) {
        await Promise.all([
          ...
          this.callCrossOriginComment(CROSS_ORIGIN_EDITOR_COMMAND.COMMENT_SUBSCRIBE, {}),
          this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_SUBSCRIBE),
        ])

        const unsubscribers = [
          subscribeCrossOriginEditorEvent(this.containerId, CROSS_ORIGIN_EDITOR_EVENT.ADD_COMMENT, ([id, data]) => {
          ...
          subscribeCrossOriginEditorEvent(this.containerId, CROSS_ORIGIN_EDITOR_EVENT.CHANGE_COMMENT, ([id, data]) => {
            const commentId = String(id)
            const commentData = data as CommentData
            if (isResolvedComment(commentData)) {
              this.comments.delete(commentId)
          ...
          subscribeCrossOriginEditorEvent(this.containerId, CROSS_ORIGIN_EDITOR_EVENT.REMOVE_COMMENT, ([id]) => {
            this.comments.delete(String(id))
          }),
          subscribeCrossOriginEditorEvent(
            this.containerId,
  ...
  private hideWordDocModeSwitcher() {
    ...
    const hedset = this.getEditorFrameWindow()?.document?.querySelector('[data-layout-name="header-editMode"]')
  ...
  private syncShellEditingDisable(disabled: boolean, documentType = getDocumentType(this.fileType)) {
  ...
  createConnector(options?: OnlyOfficeConnectorOptions): OnlyOfficeConnector {
    ...
    const frameEditorId = iframeUrl.searchParams.get('frameEditorId') ?? 'iframeEditor'
  ...
  private pendingUserSaveSnapshot: ReturnType<EditorServer['getDocumentSnapshot']> | null = null

  /** 用户保存：更新快照并广播 SAVE_DOCUMENT + ONSAVE（同 tick 内合并重复回调）。 */
  private notifyUserSave(snapshot?: ReturnType<EditorServer['getDocumentSnapshot']>) {
    if (snapshot) {
    ...
    this.userSaveTimer = window.setTimeout(() => {
      ...
      const snap = this.pendingUserSaveSnapshot ?? this.server.getDocumentSnapshot()
  ...
  private isLoadSessionActive(containerId: string, loadSession?: number) {
    return loadSession === undefined || editorManagerFactory.isLoadSessionActive(containerId, loadSession)
  }
  ...
  renameDocument(fileName: string): Promise<string> {
    ...
    if (!requestedName) {
      return Promise.reject(new Error('OnlyOffice document name cannot be empty'))
    }
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      try {
        if (isOnlyOfficeCdnMode()) {
          void callCrossOriginEditor(this.containerId, CROSS_ORIGIN_EDITOR_COMMAND.DOCUMENT_RENAME, {
  ...
  private rejectPendingRename(error: unknown) {
    ...
    window.clearTimeout(pendingRename.timer)
    this.pendingRename = null
    pendingRename.reject(error instanceof Error ? error : new Error(String(error)))
  }
  ...
  async subscribe({ type, fn }: { type: AscWordApiMethod; fn: AscWordApiCallback }) {
    if (isOnlyOfficeCdnMode()) {
      if (
        ...
        await this.callCrossOriginComment(CROSS_ORIGIN_EDITOR_COMMAND.COMMENT_SUBSCRIBE, {})
        return subscribeCrossOriginEditorEvent(this.containerId, type, (args) => fn(...args))
      }
      ...
      if (
        ...
        await this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_SUBSCRIBE)
        return subscribeCrossOriginEditorEvent(this.containerId, type, (args) => fn(...args))
      }

      if (type === CROSS_ORIGIN_EDITOR_EVENT.DOCUMENT_MODIFIED_CHANGED) {
        await callCrossOriginEditor(this.containerId, CROSS_ORIGIN_EDITOR_COMMAND.EDITOR_SUBSCRIBE, { event: type })
        return subscribeCrossOriginEditorEvent(this.containerId, type, (args) => fn(...args))
      }

      throw new Error(`OnlyOffice cross-origin callback is not supported: ${type}`)
    }
  ...
  async callCrossOriginComment(command: string, payload: Record<string, unknown>) {
    return callCrossOriginEditor(this.containerId, command, payload)
  }
  ...
  addComment(input: CommentInput) {
    if (isOnlyOfficeCdnMode()) {
      ...
      return this.callCrossOriginComment(CROSS_ORIGIN_EDITOR_COMMAND.COMMENT_ADD, { data }).then((id) => {
        if (id) {
    ...
    const data = toPluginCommentPayload(normalizeCommentInput(input))
    const id =
      api.pluginMethod_AddComment?.(data) ?? api.asc_addComment?.(this.createSdkCommentPayload(data) as CommentData)
    if (id) {
      this.comments.set(String(id), data)
    }
    return id ? String(id) : ''
  ...
  updateComment(id: string, data: CommentData) {
    ...
    if (isOnlyOfficeCdnMode()) {
      const payload = toPluginCommentPayload(data)
      return this.callCrossOriginComment(CROSS_ORIGIN_EDITOR_COMMAND.COMMENT_UPDATE, {
        id,
        data: payload,
      }).then(() => {
    ...
    if (typeof api.pluginMethod_ChangeComment === 'function') {
      ...
      api.asc_changeComment?.(id, this.createSdkCommentPayload(payload) as CommentData)
    }
    this.comments.set(id, payload)
  }
  ...
  removeComment(id: string) {
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginComment(CROSS_ORIGIN_EDITOR_COMMAND.COMMENT_REMOVE, { id }).then(() => {
  ...
  goToComment(id: string, { showBalloon = false }: { showBalloon?: boolean } = {}) {
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginComment(CROSS_ORIGIN_EDITOR_COMMAND.COMMENT_GO_TO, { id, showBalloon })
    }
  ...
  setTrackRevisions(enabled: boolean) {
    ...
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_SET_TRACK, { enabled })
    }
  ...
  prepareRevisionReview() {
    this.trackRevisions = true
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_PREPARE_REVIEW).then((items) => {
        this.revisions = this.normalizeCrossOriginRevisions(items)
      })
    }
  ...
  isTrackRevisions() {
    if (isOnlyOfficeCdnMode()) {
      void this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_IS_TRACK).then((enabled) => {
        this.trackRevisions = !!enabled
      })
  ...
  haveRevisionsChanges() {
    if (isOnlyOfficeCdnMode()) {
      void this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_HAVE_CHANGES).then((hasChanges) => {
        if (!hasChanges) {
          this.revisions = []
        } else {
  ...
  goToNextRevision() {
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_NEXT)
    }
  ...
  goToPrevRevision() {
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_PREV)
    }
  ...
  goToRevision(id: string) {
    if (isOnlyOfficeCdnMode()) {
      ...
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_GO_TO, {
        id,
        index: cached?.Index,
      })
    }
    ...
    goToRevisionInSdk(cached ?? id, api as RevisionsEditorApi, frameWin, this.revisions)
  }
  ...
  acceptRevision(revision: RevisionItem | string) {
    if (isOnlyOfficeCdnMode()) {
      ...
      const cached = this.revisions.find((entry) => entry.Id === id)
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_ACCEPT, {
        id,
        index: cached?.Index,
      }).then((items) => {
        this.revisions = this.normalizeCrossOriginRevisions(items)
    ...
    if (applyRevisionChange('accept', revision, api as RevisionsEditorApi, frameWin, this.revisions)) {
      this.syncRevisionsAfterMutation()
    }
  }
  ...
  rejectRevision(revision: RevisionItem | string) {
    if (isOnlyOfficeCdnMode()) {
      const id = this.revisionTargetId(revision)
      const cached = this.revisions.find((entry) => entry.Id === id)
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_REJECT, {
        id,
        index: cached?.Index,
      }).then((items) => {
        this.revisions = this.normalizeCrossOriginRevisions(items)
    ...
    if (applyRevisionChange('reject', revision, api as RevisionsEditorApi, frameWin, this.revisions)) {
      this.syncRevisionsAfterMutation()
    }
  }
  ...
  acceptRevisionsBySelection(all?: boolean) {
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_ACCEPT_SELECTION, { all }).then(
        (items) => {
          this.revisions = this.normalizeCrossOriginRevisions(items)
        },
      )
  ...
  rejectRevisionsBySelection(all?: boolean) {
    if (isOnlyOfficeCdnMode()) {
      return this.callCrossOriginRevision(CROSS_ORIGIN_EDITOR_COMMAND.REVISION_REJECT_SELECTION, { all }).then(
        (items) => {
          this.revisions = this.normalizeCrossOriginRevisions(items)
        },
      )
      ...
      handlers.onShowChanges
        ? this.subscribe({
            ...
            fn: (stack) => {
              ...
              handlers.onShowChanges?.(resolveRevisionShowChanges(stack, api as RevisionsEditorApi, frameWin))
            },
          })
        : undefined,
