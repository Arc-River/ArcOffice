...
export type OfficeThemeId = (typeof OFFICE_THEME)[keyof typeof OFFICE_THEME]
...
export const OFFICE_EDITOR_LOGO = {
  /** 浅色主题：Office 品牌色图标 */
  image: 'https://cdn.jsdelivr.net/npm/simple-icons@9.21.0/icons/microsoftoffice.svg',
  /** 深色主题：同图标（品牌色在深色背景上同样清晰） */
  imageDark: 'https://cdn.jsdelivr.net/npm/simple-icons@9.21.0/icons/microsoftoffice.svg',
} as const
...
function buildStaticResource(): StaticResource {
  ...
  const onlyofficeRoot = cdnOrigin ? `${cdnOrigin}/onlyoffice/${onlyofficeVersion}` : DEFAULT_ONLYOFFICE_ROOT
...
export function registerOnlyOfficeStaticResource(options: OnlyOfficeStaticResourceOptions): StaticResource {
