...
type WordLogicDocument = LogicDocumentWithRevisions & {
  ...
  Ihc?: (moveId: string, fromMove: boolean, scroll?: boolean, select?: boolean) => void
  Um?: RevisionUm
  Wq?: RevisionWq
}
...
export type RevisionsEditorApi = {
  ...
  asc_GetTrackRevisionsReportByAuthors?: () => Record<string, SdkRevisionElement[]>
...
function getLogicDocument(api: RevisionsEditorApi): WordLogicDocument | null {
...
function isSameRevision(a: SdkRevisionElement, b: SdkRevisionElement): boolean {
  if (a === b) return true
...
function resolveFreshRevisionRaw(doc: WordLogicDocument, raw: SdkRevisionElement): SdkRevisionElement {
...
function applyRevisionSelection(doc: WordLogicDocument, raw: SdkRevisionElement, frameWin: Window): boolean {
  ...
  const element = raw.Element ?? (typeof raw.ee === 'function' ? raw.ee() : undefined)
  const start = typeof raw.wa === 'number' ? raw.wa : revisionStart(raw)
  if (element == null || start == null) return false

  const end = typeof raw.xa === 'number' ? raw.xa : (revisionEnd(raw) ?? start)
...
function dismissReviewChangesDialog(frameWin?: Window | null): void {
 ...
 */
export function prepareRevisionReviewDisplay(api: RevisionsEditorApi, frameWin?: Window | null): void {
  ...
  if (typeof api.asc_SetDisplayModeInReview === 'function') {
    ...
    const markup = Asc?.c_oAscReviewDisplay?.Edit ?? Asc?.c_oAscReviewDisplay?.Markup ?? Asc?.Xja?.AKa
    if (markup !== undefined) {
...
function revisionChangeToPlain(raw: SdkRevisionElement, enums: AscRevisionEnums): RevisionData {
  ...
  return {
    ...
    UserName:
      (typeof raw.get_UserName === 'function' ? raw.get_UserName() : undefined) ??
      (typeof raw.wV === 'function' ? raw.wV() : undefined),
    DateTime:
      (typeof raw.get_DateTime === 'function' ? raw.get_DateTime() : undefined) ??
...
function collectRevisionRawsFromWq(doc: WordLogicDocument): SdkRevisionElement[] {
  const wq = doc.Wq
  const qt = wq?.wih?.()
  ...
  for (const el of wq?.Tpa ?? []) {
    const id = typeof el.Yb === 'function' ? String(el.Yb()) : typeof el.ed === 'function' ? String(el.ed()) : ''
  ...
  for (const id of orderedIds.length > 0 ? orderedIds : Object.keys(qt)) {
    const group = qt[id]
    if (!Array.isArray(group)) continue
    raws.push(...(group.length > 1 ? [...group].sort((a, b) => revisionSortKey(a) - revisionSortKey(b)) : group))
  }

  return raws
}

function readRevisionStack(api: RevisionsEditorApi, allowSyncEnd: boolean, forceRefresh = false): SdkRevisionElement[] {
  if (!forceRefresh) {
    const existing = (api.asc_GetRevisionsChangesStack?.() ?? []) as SdkRevisionElement[]
...
export function collectRevisionItems(
  ...
  const enums = getAscRevisionEnums(frameWin)
  const revId = (raw: SdkRevisionElement, i: number) => `rev-${typeof raw.ed === 'function' ? String(raw.ed()) : i}`
...
export function resolveRevisionShowChanges(stack: unknown, api: RevisionsEditorApi, frameWin: Window): RevisionItem[] {
...
function findRevisionItem(items: RevisionItem[], id: string): RevisionItem | undefined {
...
function resolveRevisionTarget(
  ...
  return findRevisionItem(collectRevisionItems(api, frameWin, { allowSyncEnd: false }), target)
}

function scrollToRevisionPosition(api: RevisionsEditorApi, data: RevisionData): void {
  if (data.Y != null) api.ViewScrollToY?.(data.Y)
  if (data.X != null) api.ViewScrollToX?.(data.X)
}

function scrollRevisionIntoView(doc: WordLogicDocument, api: RevisionsEditorApi, data: RevisionData): void {
...
function activateRevision(doc: WordLogicDocument, raw: SdkRevisionElement): void {
  const wq = doc.Wq
  if (!wq) return
...
function isMoveRevision(raw: SdkRevisionElement, frameWin: Window): boolean {
  const enums = getAscRevisionEnums(frameWin)
  const moveType =
    (typeof raw.get_MoveType === 'function' ? raw.get_MoveType() : undefined) ??
    (typeof raw.hZd === 'function' ? raw.hZd() : undefined)
