import { resolveSiteUrl, STATIC_RESOURCE, X2T_PDF_FONT_MANIFEST } from '../../const'
...
function isJavaScriptSource(data: Uint8Array) {
  const head = new TextDecoder().decode(data.slice(0, Math.min(data.length, 128)))
...
async function decompressWithStream(buffer: ArrayBuffer) {
  const stream = new Blob([buffer]).stream().pipeThrough(new DecompressionStream('br' as CompressionFormat))
 ...
 */
export async function fetchMaybeBrotliAsset(url: string, fetchImpl: typeof fetch = fetch) {
 ...
 */
export async function fetchMaybeBrotliScript(url: string, fetchImpl: typeof fetch = fetch) {
...
async function fetchPdfFontFile(origin: string, root: string, file: string) {
  const url = resolveSiteUrl(origin, `${root}/${file}`)
...
export async function loadX2tPdfFonts(
  ...
  loadingPromise = (async () => {
    ...
    if (!fonts['Carlito.ttf']?.byteLength) {
      console.warn('[x2t-assets] Carlito regular missing under', root)
      return {}
    }
