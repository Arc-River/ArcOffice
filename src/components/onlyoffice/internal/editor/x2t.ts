...
import type { X2tConvertParams, X2tConvertResult } from './types'
...
export class X2tConverter {
  ...
  private logRaw(level: 'log' | 'error', message: string, consoleArgs: unknown[]) {
    if (this.logger) {
  ...
  private handleWorkerError = (error: ErrorEvent) => {
    this.logRaw('error', 'worker error', ['[X2tConverter] Worker error:', error])
  ...
  public async convert(
    ...
    const cloneMap = (map?: { [key: string]: Uint8Array }) => {
      ...
      return Object.fromEntries(Object.entries(map).map(([key, value]) => [key, value.slice(0)]))
    }
  ...
  public terminate(logger?: EditorLogger): void {
    ...
    if (this.worker) {
      ...
      this.worker = null
      this.initPromise = null
      this.logRaw('log', 'worker terminated', ['[X2tConverter] Worker terminated'])
    }
  }
