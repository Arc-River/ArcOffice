...
function buildCentralHeader(
  entry: Pick<ZipEntry, 'nameBytes' | 'method' | 'modTime' | 'modDate' | 'internalAttrs' | 'externalAttrs'>,
  crc: number,
  compressedSize: number,
  uncompressedSize: number,
  localOffset: number,
...
export function getZipXmlUncompressedSize(entries: readonly ZipEntry[]): ZipXmlSizeStats {
  let totalSize = 0
  let entryCount = 0
...
export function writeZipEntries(entries: ZipEntry[], replacements: Map<string, ZipReplacement>) {
  ...
  const emit = (
    ...
    const uncompressedSize = replacement ? outputData.length : entry.uncompressedSize
    ...
    const localHeader = buildLocalHeader(nextEntry, crc, outputData.length, uncompressedSize)
    localParts.push(localHeader, outputData)
    centralParts.push(buildCentralHeader(nextEntry, crc, outputData.length, uncompressedSize, outputOffset))
...
function xml(strings: TemplateStringsArray, ...values: unknown[]) {
  return strings.reduce((result, value, index) => `${result}${value}${values[index] ?? ''}`, '').trim()
}
