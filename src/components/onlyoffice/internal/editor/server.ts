import { isOnlyOfficeCdnMode, OFFICE_XML_EVENT_CONFIG, type OfficeXmlEventConfig } from '../../const'
...
import type { MockSocket } from './runtime-bridge'
import {
  AscSaveTypes,
...
function isOfficeZipFileType(fileType: string) {
  return /^(docx|docm|dotx|dotm|xlsx|xlsm|xltx|xltm|pptx|pptm|potx|potm|ppsx|ppsm)$/i.test(getFileExt(fileType))
}
...
type CoAuthoringLockBlock = string | number | { guid?: string; [key: string]: unknown }
...
function getCoAuthoringLockKey(block: CoAuthoringLockBlock, isSpreadsheet: boolean) {
  if (isSpreadsheet && typeof block === 'object' && block !== null && block.guid != null) {
    return String(block.guid)
  }
  return String(block)
}

function buildCoAuthoringLocks(block: unknown, fileType: string, userId?: string) {
  const isSpreadsheet = getDocumentType(fileType) === 'cell'
  const time = Date.now()
  const locks: Record<string, { time: number; user?: string; block: CoAuthoringLockBlock }> = {}
...
function ensurePngContentType(contentTypes: string) {
  ...
  return contentTypes.replace('</Types>', '<Default Extension="png" ContentType="image/png"/></Types>')
}
...
function parseSvgSize(svgText: string) {
  ...
  const viewBox = /<svg\b[^>]*\bviewBox="([^"]+)"/i.exec(svgText)?.[1]?.trim().split(/\s+/).map(Number)
...
function replaceAsciiExtension(data: Uint8Array, fromExt: string, toExt: string) {
  return replaceBytes(data, new TextEncoder().encode(fromExt), new TextEncoder().encode(toExt))
}

function replaceUtf16LeExtension(data: Uint8Array, fromExt: string, toExt: string) {
...
function _getArrayField(record: Record<string, unknown> | null, key: string) {
  const value = record?.[key]
...
function collectTextFragments(value: unknown, fragments: string[] = [], depth = 0) {
  ...
  if (typeof value === 'string') {
    ...
    if (text.length > 0 && text.length <= 200 && /[\p{L}\p{N}\u4e00-\u9fff]/u.test(text)) {
  ...
  if (typeof value === 'object') {
    const record = value as Record<string, unknown>
    const preferredKeys = ['text', 'Text', 'value', 'insert', 'm_sText', 'data', 'items']
    const keys = [
...
function summarizeChangePacket(value: unknown) {
  ...
  return {
    ...
    byteLength,
    byteLengthMatches:
      Number.isFinite(declaredLength) && byteLength != null ? declaredLength === byteLength : undefined,
...
function summarizeSaveChanges(record: Record<string, unknown>) {
  ...
  const parsedFirstChange = typeof firstChange === 'string' ? tryParseJson(firstChange) : firstChange
  const textPreview = collectTextFragments(changes ?? firstChange)
  const packets = changes?.map(summarizeChangePacket)
  const changeBytes =
    packets?.reduce((total, packet) => total + (packet.byteLength ?? 0), 0) ?? getPayloadByteLength(firstChange)
...
function summarizeWsPayload(payloads: unknown[]) {
  ...
  if (type === 'saveChanges' && record) {
    ...
    if (saveSummary.start || saveSummary.end) {
      extraParts.push(`${saveSummary.start ? 'start' : ''}${saveSummary.end ? '+end' : ''}`)
    }
  ...
  } else if ((type === 'getLock' || type === 'releaseLock') && record?.locks && typeof record.locks === 'object') {
  ...
  const labelParts = [type ?? typeof first, dataType, getStringField(data, 'status')].filter(Boolean)
  const label = [labelParts.join(' / '), extraParts.length > 0 ? `(${extraParts.join(', ')})` : '']
    .filter(Boolean)
    .join(' ')
...
function summarizeDownloadCommand(cmd: Record<string, unknown>, buffer: ArrayBuffer) {
  const outputFormat = getNumberField(cmd, 'outputformat')
  const filetype = outputFormat == null ? undefined : extensionFromOutputFormat(outputFormat)
  const saveType = getSaveTypeLabel(cmd.savetype)

  return {
    label: [saveType, filetype ? `to ${filetype}` : undefined, cmd.isSaveAs ? 'saveAs' : undefined]
      .filter(Boolean)
      .join(' '),
    saveType,
    outputFormat,
...
export class EditorServer {
  ...
  private messageHandlers = new WeakMap<MockSocket, (msg: unknown, ...args: unknown[]) => void>()
  ...
  private createLoadPromise(buffer: ArrayBuffer | (() => Promise<ArrayBuffer>), fileType: string) {
  ...
  async open(file: File, { fileType, fileName }: { fileType?: string; fileName?: string } = {}) {
    ...
    const sizeLimitError = this.getOfficeXmlSizeLimitError(buffer, this.fileType)
    if (sizeLimitError) {
      this.loadBlocked = true
  ...
  openNew(fileType?: string, fileName?: string) {
    ...
    this.id = randomId()
    this.loadPromise = null
    this.title = ensureTitleWithExtension(fileName || 'New Document', this.fileType)
    const documentType = getDocumentType(this.fileType)
  ...
  async openUrl(
    ...
    const rawTitle = fileName || decodeURIComponent(url.split('/').pop() || 'Document')
  ...
  private getStoredMedia() {
    return Object.fromEntries(Array.from(this.fsMap.entries()).filter(([key]) => key.startsWith('media/')))
  }

  private getStoredThemes() {
    return Object.fromEntries(Array.from(this.fsMap.entries()).filter(([key]) => key.startsWith('themes/')))
  }
  ...
  private getExportSafeEditorBin(binData: Uint8Array) {
    const hasUnsupportedVectorMedia = Object.keys(this.getStoredMedia()).some((key) => /\.(emf|svg)$/i.test(key))
  ...
  commitUserSave(data: Uint8Array) {
    if (!isValidEditorBin(data)) {
      console.warn('[EditorServer] Ignoring invalid Editor.bin from save, length:', data.length)
      return
    }
  ...
  async captureCurrentDocument(
    ...
    if (this.pendingExport) {
      window.clearTimeout(this.pendingExport.timer)
      this.pendingExport.reject(new DOMException('OnlyOffice export was superseded', 'AbortError'))
      this.pendingExport = null
    }
   ...
   */
  private async loadDocument(buffer: ArrayBuffer | (() => Promise<ArrayBuffer>), fileType: string) {
    ...
    const fallback =
      isOfficeZipFileType(fileType) && !isZipBytes(bytes) ? createPlainTextOfficeFallback(buffer, fileType) : null
    if (fallback) {
    ...
    if (sourceFileType === 'pdf') {
      ...
      ;({ output, media, themes } = await this.convertBufferToEditorBin(buffer, sourceFileType))
    }
  ...
  private async rewriteUnsupportedPptxImages(buffer: ArrayBuffer, fileType: string) {
    ...
    const mediaFiles = entries.filter(
      (entry) => !entry.name.endsWith('/') && /^ppt\/media\/[^/]+\.(emf|svg|webp)$/i.test(entry.name),
    )
    ...
    const relFiles = entries.filter((entry) => !entry.name.endsWith('/') && entry.name.endsWith('.rels'))
    for (const relFile of relFiles) {
      ...
      for (const [from, to] of replacements) {
        ...
        const nextText = text.replace(new RegExp(escapeRegExp(fromName), 'g'), toName)
        if (nextText !== text) {
          text = nextText
          changed = true
        }
    ...
    if (contentTypes) {
      ...
      if (source) {
        entryReplacements.set(contentTypes.name, {
          data: new TextEncoder().encode(ensurePngContentType(new TextDecoder().decode(source))),
   ...
   */
  private storeDownloadOutput(outputName: string, data: Uint8Array, downloadFileName: string) {
    this.fsMap.set(outputName, data)
    this.downloadFileNames.set(outputName, downloadFileName)
    const mimeType = getOnlyOfficeMimeType(getFileExt(outputName.replace(/^output\./, '')))
   ...
   */
  private isDownloadAsOutput(cmd: Record<string, unknown> | null, input: Uint8Array) {
    ...
    const outputFormat = typeof cmd?.outputformat === 'number' ? cmd.outputformat : undefined
  ...
  private async convertEditorBinToPdf(binData: Uint8Array, pdfRendererStream?: Uint8Array) {
    binData = this.getExportSafeEditorBin(binData)
    const media = this.getExportSafeMedia()
    const themes = this.getStoredThemes()
  ...
  private async resolveDownloadOutputData(input: Uint8Array, filetype: string) {
    ...
    if (ext === 'pdf') {
      ...
      try {
        ...
        throw err instanceof Error ? err : new Error('PDF export failed', { cause: err })
      }
    }
  ...
  private resolveDownloadFileType(cmd: Record<string, unknown> | null) {
    const fromOutput = extensionFromOutputFormat(typeof cmd?.outputformat === 'number' ? cmd.outputformat : undefined)
    if (fromOutput) {
  ...
  private resolveDownloadFileName(cmd: Record<string, unknown> | null, filetype: string) {
  ...
  private resolvePendingExport(data: Uint8Array) {
     ...
     */
    if (!isValidEditorBin(data)) {
      pendingExport.reject(new Error('OnlyOffice export returned invalid document data'))
      return true
    }
  ...
  private async handleImgUrls(socket: MockSocket, message: Record<string, unknown>) {
  ...
  private sendTo(socket: MockSocket, ...msg: unknown[]) {
    ...
    this.logRaw('log', 'socket', `ws outgoing: ${summary.label}`, ['[ws] >> ', ...msg], summary)
  ...
  async handleMessage(socket: MockSocket, msg: Record<string, unknown>, ...args: unknown[]) {
    const incomingSummary = summarizeWsPayload([msg, ...args])
    this.logRaw('log', 'socket', `ws incoming: ${incomingSummary.label}`, ['[ws] << ', msg, args], incomingSummary)

    const send = (...payload: unknown[]) => this.sendTo(socket, ...payload)
    const { sessionId, participants, user, client } = this
    const type = typeof msg === 'object' && msg && 'type' in msg ? msg.type : null
    switch (type) {
      case 'auth': {
      ...
      case 'getLock':
        ...
        {
          const locks = buildCoAuthoringLocks(msg.block, this.fileType, user?.id)
      ...
      case 'rpc': {
        ...
        try {
          ...
          const name = extension ? title.slice(0, -(extension.length + 1)) : title

          // asc_wopi_renameFile 期望 { Name }，SDK 会自行补回当前扩展名并广播 asc_onMeta。
          send({ type: 'rpc', responseKey, data: { Name: name } })
        } catch (error) {
          this.logRaw('warn', 'socket', 'wopi rename failed', ['[EditorServer] wopi rename failed:', error])
  ...
  async handleRequest(req: Request) {
    ...
    if (cacheMatch && req.method === 'GET') {
      ...
      const downloadName = getCacheDownloadName(outputName, this.title, filetype, this.downloadFileNames)
    ...
    if (u.pathname.endsWith(`/downloadas/${key}`)) {
      const cmd = JSON.parse(u.searchParams.get('cmd') || '{}') as Record<string, unknown>
      ...
      if (cmd.savetype === AscSaveTypes.PartStart || cmd.savetype === AscSaveTypes.CompleteAll) {
      ...
      const download = async (): Promise<DownloadResult> => {
        ...
        if (this.isDownloadAsOutput(cmdSnapshot, input)) {
          ...
          const downloadFileName = this.resolveDownloadFileName(cmdSnapshot, filetype)
          const outputData = await this.resolveDownloadOutputData(input, filetype)
          const downloadUrl = this.storeDownloadOutput(outputName, outputData, downloadFileName)
          this.endSaving()
          return {
            status: 'ok',
            isExport: false,
      ...
      const finalizeDownload = async () => {
        try {
          ...
          this.logRaw('error', 'download', 'downloadAs failed', ['[EditorServer] downloadAs failed:', err])
          this.endSaving()
          result = {
            status: 'err',
            isExport: false,
