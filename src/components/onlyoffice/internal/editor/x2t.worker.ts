...
import { AvsFileType, type X2tConvertParams, type X2tConvertResult } from './types'
import { fetchMaybeBrotliAsset, fetchMaybeBrotliScript, loadX2tPdfFonts } from './x2t-assets'
...
function rewriteFontText(text: string, aliases?: Record<string, string>) {
  ...
  for (const [from, to] of entries) {
     ...
     */
    const suffix =
      to.startsWith(from) && to.length > from.length ? to.slice(from.length).replace(/[.*+?^${}()|[\]\\]/g, '\\$&') : ''
...
function replaceBytesWithPadding(data: Uint8Array, from: Uint8Array, to: Uint8Array, skipSuffix?: Uint8Array) {
...
function restoreEditorBinFontNames(data: Uint8Array, restoreAliases?: Record<string, string>) {
  if (!restoreAliases) return data

  const entries = Object.entries(restoreAliases).sort((a, b) => b[0].length - a[0].length)

  let output = data
  for (const [from, to] of entries) {
    const skipSuffix = to.startsWith(from) && to.length > from.length ? encodeUtf16Le(to.slice(from.length)) : undefined
    output = replaceBytesWithPadding(output, encodeUtf16Le(from), encodeUtf16Le(to), skipSuffix)
  }
  return output
}
...
async function rewriteZipFontNames(data: ArrayBuffer, fileFrom: string, fontAliases?: Record<string, string>) {
  if (!fontAliases || !hasOfficeZipExtension(fileFrom)) {
    return data
  }

  return rewriteOfficeZipText(data, shouldRewriteZipEntry, (text) => rewriteFontText(text, fontAliases))
}
...
async function sanitizeDocxForX2t(data: ArrayBuffer, fileFrom: string) {
  ...
  return rewriteOfficeZipText(data, shouldRewriteDocxBookmarkEntry, normalizeDocxBookmarkIds)
}
...
function formatSerializedWorkerError(error: Omit<SerializedWorkerError, 'text'>) {
  ...
  const details = [
    ...
    .join(', ')
  return details ? `${error.name}${message} (${details})` : `${error.name}${message}`
}
...
function serializeWorkerError(error: unknown): SerializedWorkerError {
  ...
  if (error && typeof error === 'object') {
    ...
    const constructorName =
      'constructor' in record && record.constructor && typeof record.constructor === 'function'
...
function writeInputs({
  ...
  const isCsvSource = formatFrom === AvsFileType.AVS_FILE_SPREADSHEET_CSV || fileFrom.toLowerCase().endsWith('.csv')
...
async function convert({
  ...
  if (needsPdfFonts) {
    ...
    const pdfFonts = await loadX2tPdfFonts(self.location.origin, pdfFontsRoot)
  ...
  try {
    ...
    if (output && hasOfficeZipExtension(fileTo) && fontExportAliases) {
      const restored = await rewriteZipFontNames(output.slice(0).buffer, fileTo, fontExportAliases)
