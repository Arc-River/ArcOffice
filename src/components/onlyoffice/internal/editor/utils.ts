...
export function getX2tConvertFormats(fileType: string) {
  ...
  const formatFrom = x2tSourceFormatByExt[ext] ?? getDefaultX2tSourceFormat(fileType)
...
export function isMultilineCsv(buffer: ArrayBuffer) {
  ...
  const delimiter = getCsvDelimiterChar(detectX2tCsvDelimiter(buffer, csvEncoding))
...
export function getX2tExportFormats(fileType: string, sourceFileType?: string) {
  ...
  const formatTo = x2tSourceFormatByExt[ext] ?? getDefaultX2tSourceFormat(source)
...
export function ensureTitleWithExtension(title: string, fileType: string): string {
  ...
  const base = dotIndex > 0 && dotIndex < title.length - 1 ? title.slice(0, dotIndex) : title
  return `${base}.${ext}`
}
