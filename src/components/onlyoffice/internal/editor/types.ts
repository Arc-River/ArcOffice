...
export type OnlyOfficeConnector = {
  ...
  connect: () => void
  disconnect: () => void
  callCommand: (command: string | (() => void), callback?: (result: unknown) => void, recalculate?: boolean) => void
...
export function isOfficeXmlSizeLimitExceededError(error: unknown): error is OfficeXmlSizeLimitExceededError {
  return (
    error instanceof OfficeXmlSizeLimitExceededError ||
    (error instanceof Error && error.name === 'OfficeXmlSizeLimitExceededError' && 'payload' in error)
  )
}
