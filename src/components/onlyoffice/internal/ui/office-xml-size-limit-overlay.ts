import { OFFICE_XML_SIZE_LIMIT_ERROR_MESSAGE, type OfficeXmlSizeLimitExceededPayload } from '../editor/types'
...
export function removeOfficeXmlSizeLimitOverlay(container: HTMLElement) {
  container.querySelector<HTMLElement>(`[${OVERLAY_ATTR}="true"]`)?.remove()
}

export function showOfficeXmlSizeLimitOverlay(container: HTMLElement, payload: OfficeXmlSizeLimitExceededPayload) {
  ...
  title.textContent = payload.errorDescription || OFFICE_XML_SIZE_LIMIT_ERROR_MESSAGE
  ...
  description.textContent = 'Office 文件内 XML 解压后超过安全阈值，请压缩或拆分文档后重试。'
