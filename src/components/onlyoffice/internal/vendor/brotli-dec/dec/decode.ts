...
import { BrotliBuildHuffmanTable, HuffmanCode } from './huffman'
...
var kMaxHuffmanTableSize = new Uint16Array([
  256, 402, 436, 468, 500, 534, 566, 598, 630, 662, 694, 726, 758, 790, 822, 854, 886, 920, 952, 984, 1016, 1048, 1080,
])
...
function BrotliDecompress(input, output) {
  ...
  while (!input_end) {
    ...
    while (meta_block_remaining_len > 0) {
      ...
      copy_length =
        Prefix.kCopyLengthPrefixCode[copy_code].offset + br.readBits(Prefix.kCopyLengthPrefixCode[copy_code].nbits)
      ...
      for (j = 0; j < insert_length; ++j) {
        ...
        context =
          Context.lookup[context_lookup_offset1 + prev_byte1] | Context.lookup[context_lookup_offset2 + prev_byte2]
